...
pub fn get_bytes_available<'gc>(
    ...
    if let Some(file_stream_object) = this.as_file_stream_object() {
        if let Some(ref mut handle) = *file_stream_object.handle().borrow_mut() {
            if let Ok(current_pos) = handle.stream_position() {
                if let Ok(length) = handle.seek(SeekFrom::End(0)) {
                    if handle.seek(SeekFrom::Start(current_pos)).is_ok() {
                        return Ok((length - current_pos).into());
                    }
                }
            }
        }
...
impl_write!(write_f32 write_float f64_to_wrapping_f32; f32, write_f64 write_double; f64, write_i32 write_int f64_to_wrapping_i32; i32,
            write_u32 write_unsigned_int f64_to_wrapping_u32; u32, write_i16 write_short f64_to_wrapping_i16; i16, write_u16 _write_unsigned_short f64_to_wrapping_u16; u16);
...
fn read_utf_bytes_inner<'gc>(
    ...
    if handle.take(length).read_to_end(&mut bytes).is_ok() {
        // strips off an optional UTF8 BOM at the beginning
        if bytes.starts_with(&[0xEF, 0xBB, 0xBF]) {
            bytes.drain(..3);
        }
        // cuts the result at the first null byte to recreate a bug in FP/AIR
        if let Some(null_pos) = bytes.iter().position(|&b| b == 0x00) {
...
pub fn read_utf<'gc>(
    ...
    if let Some(file_stream_object) = this.as_file_stream_object() {
        ...
        if let Some(ref mut handle) = *file_stream_object.handle().borrow_mut() {
            ...
            return read_utf_bytes_inner(activation, handle, length as u64);
        }
    }
