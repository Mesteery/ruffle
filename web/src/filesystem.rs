...
impl FileSystemBackend for VirtualFileSystemStorageBackend {
    ...
    fn open(&mut self, path: &Path, _mode: FileOpenMode) -> Result<Box<dyn File>> {
        ...
        if path.ends_with("log4as.xml") {
            return Ok(Box::new(NullFile(Cursor::new(
                r#""<logging><targets><target module="com.ankamagames.jerakine.logger.targets.FileTarget"/></targets></logging>""#
                    .as_bytes()
                    .to_vec(),
            ), path.into())));
        }
